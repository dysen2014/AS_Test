 ...
 * 到期提醒
 */
public class ExpirationWarnFragment extends ParentFragment implements BaseRefreshListener {
    ...
    private List<BeanHeader> expirationReminderListHeader = new ArrayList<>();
    ...
    Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.getData() != null) {
                ...
                progressLoading.setVisibility(View.GONE);
                expirationReminderListHeader = parseListHeader(bundle.getString("header"));
//                List<ExpirationReminder> list = parseList(bundle.getString("data"));
//
...
//                }
    ...
    protected List<ExpirationReminder> parseList(String jsonData) throws JsonSyntaxException {

//        LogUtils.d("------------------jsonData:" + jsonData);
        if (!TextUtils.isEmpty(jsonData) || jsonData != null) {
            Gson gson = new Gson();

            List<ExpirationReminder> list = gson.fromJson(jsonData, new TypeToken<List<ExpirationReminder>>() {
            }.getType());
    ...
    private void initData() {
        ...
        mListTop = InitData.getTopHeaderData(expirationReminderListHeader);
        if (checkObjValid(expirationReminderList)) {
            checkDataValid(expirationReminderList.size());
            mListLeft = InitData.getLeftHeaderData(expirationReminderList.size());
            mListData = InitData.ExpirationWarn.getCellData(expirationReminderListHeader, expirationReminderList);
        }
//		if (topName.size() > 0) {
...
//				setSelectorItemColor(view, R.color.colorAccent);
    ...
    private void clearList() {
        ...
        expirationReminderList.clear();
        expirationReminderListHeader.clear();
        mListData.clear();
        mListLeft.clear();
        mListTop.clear();
    }
