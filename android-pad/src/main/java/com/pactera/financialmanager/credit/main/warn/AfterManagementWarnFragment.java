...
public class AfterManagementWarnFragment extends ParentFragment implements BaseRefreshListener {
    ...
    private int index1;
    int CurPage = 1;
    private String QueryOrgId, OrgType, next;
    ...
    private List<BeanHeader.AfterManagementBean> listHeader = new ArrayList<>();
...
//                    toast("请求超时！");
    ...
    private void parseData(Message msg) {
        ...
        try {
            ...
            listHeader = HttpThread.parseList(jsonObject.getJSONArray
                    ("header").toString(), BeanHeader.AfterManagementBean.class);
            ...
            LogUtils.i(mListData+"------------"+mListData.size());
            if (mListData != null )
                initView();
    ...
    public void setCodeType(String isForperson) {
        this.CodeType = isForperson;
    }
    private int count;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        view = inflater.inflate(R.layout.fragment_later_warn, null);
    ...
    private void sendRequest() {
        ...
        if (CodeType.equals("yqdk")){
            //    逾期贷款
            CorrespondingCode = "crmOverdueCustomer";
            max = "100";
            min = "Min";
            HttpRequest(max, min,ParamUtils.UserId);
        }else  if (CodeType.equals("jidq")){
            //   即将到期
            CorrespondingCode = "crmExpirationReminder";
            HttpRequest();
        }else  if (CodeType.equals("dhgl")){
            //    贷后管理
            CorrespondingCode = "crmInspectRoutineHandle";
            HttpRequest(QueryOrgId, OrgType);
        }else  if (CodeType.equals("yjtx")){
            //    预警特许
            CorrespondingCode = "crmTXRemind";
            HttpRequest(QueryOrgId, OrgType);
        }else  if (CodeType.equals("detx")){
            //    贷款提醒
            OrgId = ParamUtils.orgId;
            if (num == 0){
//                CorrespondingCode = "crmBeOverdue";
                CorrespondingCode = "crmOverdueCustomer";
                HttpRequest(max, min,ParamUtils.UserId);
            }else if (num == 1){
                CorrespondingCode = "crmExpirationReminder";
                ...
                CorrespondingCode = "crmCreditRemind";
                HttpRequest(OrgId);
            }
     ...
     * @param min
     */
    private void HttpRequest(String max, String min,String userid) {

        JSONObject jsonObject = ParamUtils.setParams("warn", CorrespondingCode,
                new Object[]{max, min, userid, ParamUtils.pageSize, CurPage}, 8);
    ...
    private void HttpRequest() {
        JSONObject jsonObject = ParamUtils.setParams("warn", CorrespondingCode,
                new Object[]{ ParamUtils.UserId, ParamUtils.pageSize, CurPage}, 7);
        HttpThread.sendRequestWithOkHttp(ParamUtils.url, jsonObject, handler);
     ...
     * @param OrgId
     */
    private void HttpRequest(String OrgId) {

        JSONObject jsonObject = ParamUtils.setParams("warn", CorrespondingCode,
                new Object[]{OrgId, ParamUtils.UserId, ParamUtils.pageSize, CurPage}, 6);
        HttpThread.sendRequestWithOkHttp(ParamUtils.url, jsonObject, RadioHandler);
     ...
     * @param OrgType
     */
    private void HttpRequest(String QueryOrgId, String OrgType){
        JSONObject jsonObject = ParamUtils.setParams("warn", CorrespondingCode,
                new Object[]{QueryOrgId, OrgType, ParamUtils.UserId, ParamUtils.pageSize, CurPage}, 3);
    ...
    private void initData() {
        ...
        mListTop = InitData.getTopHeaderDataAfter(listHeader);
...
//                    item_tv.setText(mListTop.get((Integer) obj) + DescText);
                ...
                setRecyclerView(lvData, 1).setAdapter(new DataAdapterNew(getActivity(), mListData, new OnItemClickCallback<Integer>() {
                    ...
                    public void onClick(View view, Integer info) {
                        ...
                        if (next.equals("1")){
                            try {
                                QueryOrgId = jsonArray.getJSONObject(info).getString("QueryOrgId");
                                OrgType = jsonArray.getJSONObject(info).getString("OrgType");
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                            mListData.clear();
                            ...
                            List<String> listDataValue = mListData.get(info);
