...
public class ImageEngine {
	...
	public boolean init(int components, int quality) {
		if ((this.mNativeImage != null) && (this.mNativeImage.initImage(this.mEngine, components, quality) == 1)) {
			return true;
		}
		return false;
	}
	...
	public boolean load(byte[] imgbuffer) {
		if ((this.mNativeImage != null)
				&& (this.mNativeImage.loadmemjpg(this.mEngine, imgbuffer, imgbuffer.length) == 1))
			return true;
		return false;
	}
}
