...
import com.ym.idcard.util.FileUtil;
...
public class OcrEngine {
	...
	private boolean doImageBCR() {
		...
		int result = this.mNativeOcr.doImageBCR(this.pEngine, this.pImage, this.ppField);
		if (result == RECOG_OK) {
	...
	private String getFieldText(int keyLanguage) {
		...
		this.mNativeOcr.getFieldText(this.pField, arrayOfByte, arrayOfByte.length);
	...
	private boolean loadImageMem(long pBuffer, int width, int height, int component) {
		if (pBuffer != 0) {
			this.pImage = this.mNativeOcr.loadImageMem(this.pEngine, pBuffer, width, height, component);
			if (this.pImage != 0L) {
	...
	private IDCard recognize(Context context, int ocrLanguage, String imagePath) throws IOException {
		return recognize(context, ocrLanguage, FileUtil.getBytesFromFile(new File(imagePath)), null);
	}
	...
	private IDCard recognize(Context context, int ocrLanguage, byte[] idCardDataA, byte[] idCardDataB) {
		...
		if (startBCR("", "", ocrLanguage, license)) {
			...
			if (idCardDataB != null) {
				IDCard idCardB = recognizing(idCardDataB, blurDetection, keyLanguage);
	...
	private boolean startBCR(String cfgPath, String dataPath, int lanuage, byte[] license) {
		...
		if (this.mNativeOcr.startBCR(this.ppEngine, StringUtil.convertUnicodeToAscii(dataPath),
	...
	public IDCard recognize(Context paramContext, String paramString) throws IOException {
	...
	public IDCard recognize(Context paramContext, byte[] paramArrayOfByte1, byte[] paramArrayOfByte2) {
